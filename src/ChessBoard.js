...
export default class ChessBoard {
	...
	prepare(tales) {
		for (let i = 0; i < this.poles.length; i++) this.poles[i] = new Array(this.poles.length);
	...
	highlight(x, y) {
		if ((x + y) % 2 == 0) this.poles[y][x].tale.style.backgroundColor = "#964b00";
	...
	deactivate(x, y) {
		if (this.poles[y][x].activated) {
			...
			if ((this.poles[y][x].x + this.poles[y][x].y) % 2 == 0) this.poles[y][x].tale.style.backgroundColor = "#a39169";
