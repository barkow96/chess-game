...
const init = () => {
	...
	resultDiv.innerHTML = 'White pieces: ' + whitePlayer.pieces + ', black pieces: ' + blackPlayer.pieces;	
}
...
export default function performAction(event) {
	...
	if (currPlayer.firstClick) {
		...
		const totalActionsPossible = calculateTotalActions(movesPossible,capturesPossible);
		...
		if (figure.name == "K") {	
			...
			for (let i=0; i<spotsNumber; i++) cb.deactivate(attackedSpots.x[i], attackedSpots.y[i]);
		...
		//ACTIVATING ONLY THESE ACTIONS THAT WOULD NOT CAUSE CHECK ON PLAYER'S KING
			...
			for (let i=0; i<actionsNumber; i++) {
				...
				if (tempFigure != "0") cb.poles[tempY][tempX].figure.setPosition(tempX,tempY);
		...
		if (enPassant.possible && figure.name == "P" && Math.abs(x-enPassant.captureCoords[0]) == 1 && Math.abs(y-enPassant.captureCoords[1]) == 1) {
	...
	//ACTIONS TRIGGERED BY THE SECOND CLICK
		...
		//IF THE SECOND CLICK IS OTHER THAN THE FIRST CLICK THEN EXECUTE THE CODE BELOW
			...
			const figure = cb.poles[currPlayer.firstClickY][currPlayer.firstClickX].figure.setPosition(x,y);
			const color = cb.poles[currPlayer.firstClickY][currPlayer.firstClickX].color;

			//HANDLING EN PASSANT CAPTURE
			if (enPassant.possible) {
				enPassant.possible = false;

				if (x == enPassant.captureCoords[0] && y == enPassant.captureCoords[1]) {
					const figX = enPassant.figureCoords[0];
					const figY = enPassant.figureCoords[1];

					cb.removeClass(cb.poles[figY][figX].figure.name,cb.poles[figY][figX].color,figX,figY);
			...
			if ((figure.name == "P" || figure.name == "R" || figure.name == "K") && figure.firstMove) figure.firstMove = false;
			...
			if (figure.name =="K" && Math.abs(currPlayer.firstClickX-x) > 1) {
				...
				cb.removeClass(cb.poles[rookY][rookX].figure.name,cb.poles[rookY][rookX].color,rookX,rookY);
				cb.poles[rookY][rookX].figure = "0";
				cb.poles[rookY][rookX].color = "0";

				
				cb.poles[rookNewY][rookNewX].figure = rook.setPosition(rookNewX,rookNewY);
				cb.poles[rookNewY][rookNewX].color = currPlayer.color;
				cb.addClass(cb.poles[rookNewY][rookNewX].figure.name,cb.poles[rookNewY][rookNewX].color,rookNewX,rookNewY);
			}
			...
			if (figure.name == "P" && (y == 0 || y == 7)) cb.poles[y][x].figure = new Queen(x,y,cb);
			...
			cb.removeClass(figure.name,color,currPlayer.firstClickX,currPlayer.firstClickY);
			...
			if (isChecked(oppPlayer)) {
				...
				const capturePrevention = attackersNumber>1 ? false : captureCheckMatePreventionPossible(x,y);
				const blockPrevention = attackersNumber>1 ? false : blockCheckMatePreventionPossible(x,y);
				const movePrevention = moveCheckMatePreventionPossible();

				if (!capturePrevention && !blockPrevention && !movePrevention) oppPlayer.checkMate = true;
...
const findKing = (player) => {
	...
	cb.poles.forEach(row => {
		row.forEach(pole => {
			if (pole.figure != "0" && pole.figure.name == "K" && pole.color == player.color) king = pole.figure;
		})
	});
...
const calculateAttackersNumber = (player) => {
	...
	for (let i=0; i<spotsNumber; i++) if (attackedSpots.x[i] == king.x && attackedSpots.y[i] == king.y) attackersNumber++;
...
const isChecked = (player) => {
	...
	for (let i=0; i<spotsNumber; i++) {
		if (cb.poles[attackedSpots.y[i]][attackedSpots.x[i]].figure.name == "K" && cb.poles[attackedSpots.y[i]][attackedSpots.x[i]].color == player.color) return true;
	}
...
const captureCheckMatePreventionPossible = (x,y) => {
	...
	for (let i=0; i<spotsNumber; i++) if (attackedSpots.x[i] == x && attackedSpots.y[i] == y) return true;
...
const blockCheckMatePreventionPossible = (x,y) => {
	...
	if (attacker.name == "B" || attacker.name == "R" || attacker.name == "Q") {
		if (Math.abs(attacker.x-king.x) <= 1 && Math.abs(attacker.y-king.y <= 1)) return false;
		else {
			const dx = (attacker.x-king.x) != 0 ? (attacker.x-king.x)/Math.abs(attacker.x-king.x) : (attacker.x-king.x);
			const dy = (attacker.y-king.y) != 0 ? (attacker.y-king.y)/Math.abs(attacker.y-king.y) : (attacker.y-king.y);
			let multip = 1;
			let X,Y;
			let movesPossible;
			let movesNumber;
			...
			while (king.x+dx*multip != attacker.x || king.y+dy*multip != attacker.y) {
				X = king.x+dx*multip;
				Y = king.y+dy*multip;

				cb.poles.forEach(row => {
					row.forEach(pole => {
						if (pole.figure != "0" && pole.color == oppPlayer.color && pole.figure.name != "K") {
							movesPossible = pole.figure.movesPossible();
							movesNumber = movesPossible.x.length;
							for (let i=0; i<movesNumber; i++) if (movesPossible.x[i] == X && movesPossible.y[i] == Y) {blockPossible = true; break;}
						}
					})
				});
...
const moveCheckMatePreventionPossible = () => {
	...
	for (let i=0; i<totalActionsNumber; i++) {
		for (let j=0; j<spotsNumber; j++) {
			if ((totalActionsPossible.x[i] == attackedSpots.x[j] && totalActionsPossible.y[i] == attackedSpots.y[j])) {
				totalActionsPossible.x.splice(i,1);
				totalActionsPossible.y.splice(i,1);
				totalActionsPossible.possible.splice(i,1);
...
const updateResult = () => {
	...
	if (chessQuotes.change) {chessQuotes.counter = chessQuotes.counter == chessQuotes.quotes.length-1 ? 0 : chessQuotes.counter+1;}
