...
const isChecked = (player) => {
	...
	for (let i = 0; i < spotsNumber; i++) {
		if (cb.poles[attackedSpots.y[i]][attackedSpots.x[i]].figure.name == "K" && cb.poles[attackedSpots.y[i]][attackedSpots.x[i]].color == player.color)
			return true;
	}
...
const updateResult = () => {
	...
	if (chessQuotes.change) {
		chessQuotes.counter = chessQuotes.counter == chessQuotes.quotes.length - 1 ? 0 : chessQuotes.counter + 1;
	}
