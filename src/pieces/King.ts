...
import type { ChessBoardType, CastlingResult, AttackedSpots, Color, Piece } from "./pieces.types";
		...
		if (this.firstMove) {
			this.cb.poles.forEach((row) => {
				row.forEach((pole) => {
					...
					if (figure !== "0" && (figure as Piece).name === "R" && pole.color === player.color && (figure as any).firstMove) {
			...
			for (const [index, rook] of castlings.rooks.entries()) {
				if (rook !== "0") {
					...
					while (this.x + dx !== (rook as Piece).x) {
						...
						if (currentFigure !== "0" && (currentFigure as Piece).name !== "K") {
							spotsTaken = true;
						}

						for (let i = 0; i < attackedSpots.x.length; i++) {
							if (attackedSpots.x[i] === this.x + dx && attackedSpots.y[i] === this.y) {
								spotsTaken = true;
							}
						}

						dx = (multip * ((rook as Piece).x - this.x)) / Math.abs((rook as Piece).x - this.x);
						multip++;
					}
